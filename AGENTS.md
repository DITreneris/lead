...
- **Interaktyvi pamoka (kanonas):** [promptanatomy.cloud](https://promptanatomy.cloud/) — EN `/`, LT `/lt/`; LT šaltinis — [index.html](index.html), statinis build — [site/index.html](site/index.html) ir `site/lt/index.html` po `npm run build`. Absoliutūs `canonical` / `og:*` / `hreflang` / `sitemap.xml`: numatyta `https://promptanatomy.cloud` ([scripts/build-locale-pages.js](scripts/build-locale-pages.js), aplinka `PUBLIC_ORIGIN`).
- **Motininė / brendo svetainė:** [www.promptanatomy.app](https://www.promptanatomy.app/) — atskiras puslapis; hero nuoroda ir CTA į programą gali vesti į jį — tai ne pamokos URL kanonas.
