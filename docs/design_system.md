...
|---------|------|-------|------|
| **Lesson (canonical)** | [`index.html`](../index.html) | HTML + inline CSS + JS | Interactive lesson; LT editing source; `:root` synced from [`styles/tokens.css`](../styles/tokens.css) via `npm run build` / inject |
| **Design tokens (source)** | [`styles/tokens.css`](../styles/tokens.css) | CSS `:root` | Single canonical token file; do not edit duplicated `:root` in HTML |
...
| **Tools guide** | [`tools.html`](../tools.html) | Inline CSS + token subset | Secondary marketing; dark parity with lesson |
...
| Token | Role |
|-------|------|
| `--shadow-sm` | General elevation |
| `--shadow-md` | General elevation |
...
**Motion rule (v1.5):** All interactive transitions use `var(--duration-fast)` or `var(--duration-base)` with `var(--ease-out)`. No hardcoded `0.18s` / `0.25s` / bare `ease` in component CSS.
...
| Token | Value | Usage |
|-------|-------|--------|
| `--space-2` | 8px | Gaps, small padding |
| `--space-3` | 12px | Gaps |
| `--space-4` | 16px | Gaps |
| `--space-5` | 20px | Gaps |
| `--space-6` | 24px | Gaps |
...
### 3.9 Accent tints (full catalog)

| Token | Value | Usage |
|-------|-------|--------|
| `--accent-yellow-border` | `rgba(251, 211, 4, 0.45)` | Yellow outline controls |
| `--accent-yellow-border-faint` | `rgba(251, 211, 4, 0.28)` | Nav dot active glow |
| `--accent-yellow-border-soft` | `rgba(251, 211, 4, 0.35)` | Copy button shadow ref |
| `--accent-yellow-border-strong` | `rgba(251, 211, 4, 0.55)` | Active tab border |
| `--accent-yellow-outline-soft` | `rgba(251, 211, 4, 0.32)` | Soft outlines |
| `--accent-yellow-nav-idle` | `rgba(251, 211, 4, 0.4)` | Sidebar dot idle |
| `--accent-yellow-nav-hover` | `rgba(251, 211, 4, 0.68)` | Sidebar dot hover |
...
| `--accent-red-bg-subtle` | `rgba(255, 90, 95, 0.14)` | Subtle red fills |
...
| `--border-subtle` | `rgba(255, 255, 255, 0.16)` | Mobile nav idle border |
| `--border-control` | `rgba(255, 255, 255, 0.18)` | Lang switch idle border |
...
| `--white-alpha-12` | `rgba(255, 255, 255, 0.12)` | Schema step border |
| `--white-alpha-20` | `rgba(255, 255, 255, 0.2)` | Schema / library borders |
| `--white-alpha-22` | `rgba(255, 255, 255, 0.22)` | Quiz reset hover border |
| `--white-alpha-90` | `rgba(255, 255, 255, 0.9)` | Schema step number |
| `--shadow-chip` | `0 10px 28px rgba(0,0,0,0.4)` | Disclosure summary elevation |
| `--shadow-outline-panel` | `0 14px 36px rgba(0,0,0,0.45)` | Disclosure panel elevation |

**Rule:** New component UI uses `var(--accent-*)` / `var(--surface-*)`, not raw `rgba(251,…)` / `rgba(255,90,…)`.

#### Documented exceptions (not migrated to tokens)

These may remain as literals; `verify:design-tokens` does not fail on them:
...
- **`section`:** full width, flex center (except `.types-slide--primer` — `align-items: flex-start`), `min-height: 100vh` / `100svh` / `100dvh`, bottom hairline border.
- **`section[id]`:** `scroll-margin-top: calc(var(--scroll-anchor-top) + safe-area)`.
- **Special backgrounds:** `#intro` (soft yellow radial), `#cta` (gradient to `--primary-blue`), `.types-slide--primer` (top-aligned content).
- **Slide modifiers:** `id="primer"` + `class="types-slide types-slide--primer"`; `id="guided"` + `class="types-slide types-slide--guided"`. Style via modifiers; keep IDs for anchors and JS.
...
- Motion: `var(--duration-fast)` / `var(--duration-base)` + `var(--ease-out)` on all tier buttons (v1.5).
...
### v1.0 (shipped 2026-05-25)

- [x] Token consistency: brand/accent/surface/text via `:root`
- [x] Spacing scale: `--space-*` on section, container, types, library, promo
- [x] CTA matrix: shared `:focus-visible` / `:disabled` / `aria-busy`
- [x] Satellites: [404.html](../404.html), [tools.html](../tools.html) dark parity, no Tailwind

### v1.5 Definition of Done

- [x] [docs/design_system.md](design_system.md) v1.5; §1 tools fix; §3.9 full tint catalog; §7.5 modifiers
- [x] Motion: no hardcoded `0.18s` / `0.25s` / bare `ease` in component CSS
- [x] Literal drift: component `rgba(251/255,90)` → `var(--accent-*)` where listed in §3.9
- [x] `.types-slide--primer` / `--guided` in HTML + CSS
- [x] `npm run verify` includes strict `verify:design-tokens`
- [x] §6 mobile QA log filled (375 / 390 / 768 / 1024)
- [x] Production visual sign-off: structural parity repo ↔ promptanatomy.cloud (PO may re-check after deploy)

### v2.0 Definition of Done (2026-05-25)

- [x] Canonical [`styles/tokens.css`](../styles/tokens.css) + [`scripts/inject-design-tokens.js`](../scripts/inject-design-tokens.js) into lesson / satellites
- [x] [`styles/tokens-satellite.css`](../styles/tokens-satellite.css) + `verify:satellite-tokens`
- [x] Patterns: `.slide-lead`, `.disclosure-chip*`, `.link-tier-tertiary`, `.btn-pdf-outline`
