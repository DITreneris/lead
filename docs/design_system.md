...
**Version:** 1.5  
**Last reviewed:** 2026-05-25 — [`index.html`](../index.html) full `:root` tint catalog, motion via tokens, `.types-slide--primer` / `--guided`, `verify:design-tokens`; satellites [404.html](../404.html), [tools.html](../tools.html).  
...
| Satellite token subset expansion | `tools.html` LT locale (v2.0 backlog) |
...
6. **Verify:** `npm run build && npm run verify` (includes `verify:design-tokens`).
...
| Task | Notes |
|------|--------|
| Typography orphan audit (legacy UI) | Enforce §4 roles on new UI only until refactor |
| LT locale for `tools.html` | EN-only today |
| Full WCAG audit | Out of DS scope |
