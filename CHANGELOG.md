...
- **LT / EN (kelias ir SEO):** `npm run build` generuoja [site/](site/) su `index.html` (LT, šaknis), `lt/index.html` ir `en/index.html`; kiekviename puslapyje — `canonical`, `hreflang` (`lt`, `en`, `x-default` → `/lt/`), ant poaplankių puslapių statiniai keliai `/assets/…` — [scripts/build-locale-pages.js](scripts/build-locale-pages.js).
